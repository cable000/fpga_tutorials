...
- Input:
  - Hardware XSA (edt_zcu111_wrapper.xsa generated in Example 1)

Find the BSP's for the Board on the Xilinx (AMD) site:
  - [ZCU111 PetaLinux BSP](https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-design-tools.html)
...
2. This example uses a [BSP]( https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-design-tools.html) to create a PetaLinux project. Ensure that you have downloaded the correct BSP (e.g. ZCU111, ZCU102, ...) for your board for PetaLinux as instructed on the PetaLinux Tools download page.

## Creating a PetaLinux Image

1. Create a PetaLinux project using the following command:

`petalinux-create -t project -s <path to the xilinx-zcu111-v2021.2-final.bsp>`

**Note**
**xilinx-zcu111-v2021.2-final.bsp** is the PetaLinux BSP for the ZCU111 Production Silicon Rev 1.0 Board.

This creates a PetaLinux project directory, **xilinx-zcu111-2021.2**.

2. Reconfigure the project with edt_zcu111_wrapper.xsa:

- The created PetaLinux project uses the default hardware setup in the ZCU111 Linux BSP. In this example, you will reconfigure the PetaLinux project based on the Zynq UltraScale+ hardware platform that you configured using the Vivado® Design Suite in Zynq UltraScale+ MPSoC Processing System Configuration.
- Copy the hardware platform edt_zcu111_wrapper.xsa to the Linux host machine.
...
`petalinux-config --get-hw-description=<path containing edt_zcu111_wrapper.xsa> --silentconfig`

3. Build the PetaLinux project:

  1. In the **<PetaLinux-project>** directory, for example, **xilinx-zcu111-2021.2**, build the Linux images using the following command:
...
![](images/linux_apps/image7.png)

The console should print Hello World.

![](images/linux_apps/image8.png)
...
In the next chapter, you will connect all points previously introduced and create a system design.
